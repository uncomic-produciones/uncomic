...
export default function LectorMangaVertical({ params }: { params: { serieid: string; capituloId: string } }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoadingw(true);
        const docRef = doc(db, 'series', params.serieid, 'capitulos', params.capituloId);
        ...
        const q = query(
          collection(db, 'series', params.serieid, 'capitulos'),
          orderBy('numero', 'desc')
        );
    ...
    fetchData();
  }, [params.serieid, params.capituloId]);
  ...
  const idx = capitulosList.findIndex(c => c.id === params.capituloId);
  ...
  const handlePrev = () => {
    ...
    if (readingMode === 'vertical') {
      if (prevChapter) router.push(`/leer/${params.serieid}/${prevChapter.id}`);
    } else {
      if (canGoPrev) setCurrentPage(currentPage - 1);
      else if (prevChapter) router.push(`/leer/${params.serieid}/${prevChapter.id}`);
    }
  };
  ...
  const handleNext = () => {
    if (loadingw) return;
    if (readingMode === 'vertical') {
      if (nextChapter) router.push(`/leer/${params.serieid}/${nextChapter.id}`);
    } else {
      if (canGoNext) setCurrentPage(currentPage + 1);
      else if (nextChapter) router.push(`/leer/${params.serieid}/${nextChapter.id}`);
    }
  };
  ...
  return (
    <div className="relative bg-black min-h-screen">
      {params.serieid && params.capituloId && <RegisterView serieId={params.serieid} capituloId={params.capituloId}/>}
      ...
      <VoteButtons tipo="capitulo" targetId={params.capituloId}/>
